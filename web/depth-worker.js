// depth-worker.js — METRIC depth (real meters) in a Web Worker, then AISprint sector_distances().
// Bundled model: Depth-Anything-V2-Metric-Indoor-Small (Levi's ONNX export), same as the desktop
// build, with the SAME per-sector computation (20th-percentile in METERS) as src/synth.py. Because
// it outputs absolute meters, "beyond far_m -> silent" works with NO calibration. It's heavier than
// a relative model (fixed 518 input) -> best on a laptop with WebGPU; edge devices need a smaller
// export. URL flags: ?backend=wasm|webgpu (force a backend), ?fresh=1 (bypass the model cache).
...
env.localModelPath = new URL('models/', self.location.href).href;   // bundled model under /web/models/
if (SP.has('fresh')) env.useBrowserCache = false;
...
const FORCE = SP.get('backend') || undefined;        // default: WebGPU first, fall back to WASM
...
async function load() {
  ...
  if (FORCE !== 'wasm') {
    ...
    catch (e) { self.postMessage({ type: 'info', message: 'WebGPU unavailable -> WASM' }); if (FORCE === 'webgpu') throw e; }
  }
...
// colorized depth thumbnail at the CAMERA's aspect ratio (so the preview matches the camera)
function depthThumb(data, H, W, camW, camH) {
  ...
  for (let y = 0; y < DH; y++) { const sy = Math.floor(y * H / DH); for (let x = 0; x < DW; x++) { const sx = Math.floor(x * W / DW);
    const t = 1 - (data[sy * W + sx] - mn) * inv;          // metric: near = small = hot
